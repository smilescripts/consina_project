...
$cek=$db->fetch_custom("select max(NO_SO) as idMaks from sales_order");
...
if (result) {
 window.location = "<?=base_admin();?>modul/sales_order/sales_order_action.php?act=batal_penjualan&NO_NOTA_PENJUALAN=<?php echo $w.$IDbaru.'-'.$KODEOUTLETUSER;?>";
}
});
...
$.ajax({
type: "POST",
url: "<?=base_admin();?>modul/sales_order/sales_order_action.php?act=input_scanner",
data: dataString,
cache: true,
...
<td><?=$isi->NAMA_KATEGORI;?></td>
        ...
        <div class="box box-solid box-primary" style="height:640px;">
			...
			$dtb=$db->fetch_custom("SELECT tmp_detail_sales_order.*,barang_outlet.* FROM tmp_detail_sales_order
		    INNER JOIN barang_outlet on barang_outlet.KODE_BARANG=tmp_detail_sales_order.KODE_BARANG");
			$i=1;
			foreach ($dtb as $isi) {
			?>
			<tr id="line_<?=$isi->KODE_BARANG;?>">
			...
			<td><?=$isi->DISKON;?> %</td>
			 <form  method="post" class="form-horizontal foto_banyak" action="<?=base_admin();?>modul/sales_order/sales_order_action.php?act=ubah_scanner">
			...
			<a class="btn btn-danger"  href="<?=base_admin();?>modul/sales_order/sales_order_action.php?act=delete&id=<?=$isi->ID_TMP_PENJUALAN;?>"><i class="glyphicon glyphicon-trash"></i></a>
			</form>
  ...
  <div class="col-lg-3" style="width:260px;height:260px" > 
    ...
    <div class="col-lg-3" style="width:260px;margin-top:60px"> 
        <div class="box box-solid box-primary">
                  ...
                  <div class="box-body">
                     <form id="input" method="post" class="form-horizontal foto_banyak" action="<?=base_admin();?>modul/sales_order/sales_order_action.php?act=simpan_penjualan" >
                     <!-- /.form-group -->
						<div class="form-group">
                        <div class="col-lg-12">
