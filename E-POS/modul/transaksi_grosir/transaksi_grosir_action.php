...
switch ($_GET["act"]) {
	...
	if($_POST["TOTAL_TAGIHAN"]==0){
		echo "<script>alert('Belum ada barang yang dipilih');window.location='../../index.php/transaksi-grosir'</script>";
		
	}
	if($_POST["TOTAL_TAGIHAN"]!=0){
    $data = array("NO_NOTA_PENJUALAN"=>$_POST["NO_NOTA_PENJUALAN"],"TANGGAL"=>$_POST["TANGGAL"],"OPERATOR_KASIR"=>$_POST["OPERATOR_KASIR"],"SUB_TOTAL_PENJUALAN"=>$_POST["TOTAL_TAGIHAN"],"TIPE_PEMBAYARAN"=>$_POST["TIPE_PEMBAYARAN"],"DISKON_PENJUALAN"=>$_POST["DISKON_PENJUALAN"],"TAX_SALES"=>$_POST["TAX_SALES"],"UANG_BAYAR"=>$_POST["UANG_BAYAR"],"UANG_KEMBALI"=>$_POST["UANG_KEMBALI"],"PELANGGAN"=>$_POST["PELANGGAN"],"CATATAN"=>$_POST["CATATAN"],"STATUS"=>$_POST["STATUS"],"DISKON_PELANGGAN"=>$_POST["DISKON_PELANGGAN"],"NAMA_BANK"=>"TIDAK ADA","NO_KARTU"=>$_POST["NO_KARTU"],"OUTLET"=>$KODEOUTLET,"JATUH_TEMPO"=>$_POST["JATUH_TEMPO"],"BIAYA_KIRIM"=>$_POST["BIAYA_KIRIM"],);
    $in_sales = $db->insert("penjualan_grosir",$data);
    
			$gettmpjual=$db->fetch_custom("select * from tmp_detail_penjualan_grosir where NO_NOTA_PENJUALAN='$_POST[NO_NOTA_PENJUALAN]'");
			...
			$inok = $db->insert("detail_penjualan_grosir",$tmpinput);	
			}
			$truncate=$db->fetch_custom("delete from tmp_detail_penjualan_grosir where NO_NOTA_PENJUALAN='$_POST[NO_NOTA_PENJUALAN]'");
		if ($in_sales=true) {
...
unlink($file);
echo "<script>alert('Penjualan Berhasil');window.location='../../index.php/transaksi-grosir'</script>";
		}
		}
		else {
			return false;
		...
		break;
	case "delete":
		$delete=$db->delete("tmp_detail_penjualan_grosir","ID_TMP_PENJUALAN",$_GET["id"]);
		if ($delete=true) {
			header('location:../../index.php/transaksi-grosir');
		...
		case "batal_penjualan":
			...
			$truncate=$db->fetch_custom("delete from tmp_detail_penjualan_grosir where NO_NOTA_PENJUALAN=
	...
	case "input_scanner":
		...
		$in = $db->insert("tmp_detail_penjualan_grosir",$data);	
		...
		if($JUMLAHUBAH > $STOK_UBAH){
			echo "<script>alert('Stok Tidak mencukupi');window.location='../../index.php/transaksi-grosir'</script>";
		}
		else{
		$itemchange = $db->fetch_single_row("tmp_detail_penjualan_grosir","ID_TMP_PENJUALAN",$value);
	    ...
	    $upa = $db->update("tmp_detail_penjualan_grosir",$data,"ID_TMP_PENJUALAN",$value);
		if ($upa=true) {
			header('location:../../index.php/transaksi-grosir');
		} else {
			return false; 
