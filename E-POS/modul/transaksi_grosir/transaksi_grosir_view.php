...
$cek=$db->fetch_custom("select max(NO_NOTA_PENJUALAN) as idMaks from penjualan_grosir");
...
if (result) {
 window.location = "<?=base_admin();?>modul/transaksi_grosir/transaksi_grosir_action.php?act=batal_penjualan&NO_NOTA_PENJUALAN=<?php echo $w.$IDbaru.'-'.$KODEOUTLETUSER;?>";
}
});
...
<td><?=$isi->NAMA_KATEGORI;?></td>
        ...
        <div class="box box-solid box-primary" style="height:640px;">
			...
			$dtb=$db->fetch_custom("SELECT tmp_detail_penjualan_grosir.*,barang_outlet.* FROM tmp_detail_penjualan_grosir
		    INNER JOIN barang_outlet on barang_outlet.KODE_BARANG=tmp_detail_penjualan_grosir.KODE_BARANG");
			$i=1;
			foreach ($dtb as $isi) {
			?>
			<tr id="line_<?=$isi->KODE_BARANG;?>">
