...
switch ($_GET["act"]) {
	...
	case "ubahdua":
		...
		$datajumlah = array("DET_JUMLAH_BAHAN"=>$JUMLAHUBAH,);

		$upa = $db->update("detail_manufacturing",$datajumlah, "URUT",$value);
	
		if ($upa=true) {
			echo "<script>;window.location='../../index.php/manufacturing/edit?ubah=$_POST[ID]'</script>";
		} else {
			return false; 
		}
		}
	...
	case "deletelagi":
		$delete2=$db->delete("detail_manufacturing","URUT",$_GET["id"]);
		if ($delete2=true) {
			echo "<script>;window.location='../../index.php/manufacturing/edit?ubah=$_POST[ID]'</script>";
		} else {
			return false; 
	...
	case "tambahupdate":
		...
		$JUMLAH = $_POST["JUMLAH"];
			...
			$simpan = "INSERT INTO `detail_manufacturing` (`URUT`,`DET_MAN_ID_MAN`,`DET_ID_BAHAN`,`DET_JUMLAH_BAHAN`)
				values('','$ID_MAN','$ID_BAHAN','$JUMLAH')";
			$fix= mysql_query($simpan);
		echo "<script>;window.location='../../index.php/manufacturing/edit'</script>";
		break;
