...
$list_table ='
                ...
                <section class="content-header">
                    <h1>
                        Kelola '.ucwords($_POST['page_name']).'
                    </h1>
                        <ol class="breadcrumb">
                        <li><a href="<?=base_index();?>"><i class="glyphicon glyphicon-home"></i>Beranda</a></li>
                        <li><a href="<?=base_index();?>'.strtolower(str_replace(" ", "-", $_POST['page_name'])).'">'.ucwords($_POST['page_name']).'</a></li>
                        <li class="active">Daftar '.ucwords($_POST['page_name']).'</li>
                    </ol>
                </section>
                ...
                <section class="content">
                    <div class="row">
                        <div class="col-xs-12">
							...
							if ($role_act["insert_act"]=="Y") {
							?>
							<a href="<?=base_index();?>'.str_replace("_", "-", $modul_name).'/tambah" class="btn btn-primary btn-flat"><i class="glyphicon glyphicon-plus-sign"></i>Tambah Data</a>
                            <?php
                            } 
							} 
							}
  ...
  $edit = \'<a href="\'.base_index()."'.str_replace(" ", "-", $_POST['page_name'])."/edit/'+aData[indek]+'"."\".'\" alt=\"Ubah\" title=\"Ubah\" class=\"btn btn-primary btn-flat\"><i class=\"glyphicon glyphicon-pencil\"></i></a>\';
  } else {".'
    $edit ="";
  }
  if ($role_act[\'del_act\']==\'Y\') {
   $del = "<span data-id=\'+aData[indek]+\' data-uri=".base_admin()."modul/'.str_replace(" ", "_", $_POST['page_name']).'/'.str_replace(" ", "_", $_POST['page_name']).'_action.php".\' alt="Hapus" title="Hapus" class="btn btn-danger hapus btn-flat"><i class="glyphicon glyphicon-trash"></i></span>\';
  } else {
    $del="";
  }
                   } 
...
var dataTable = $("#'.$modul_name.'_dtb").dataTable({
     ...
     $('td:eq('+indek+')', nRow).html(' <a href=\"<?=base_index();?>".str_replace(" ", "-", $_POST['page_name'])."/detail/'+aData[indek]+'\" alt=\"Detail\" title=\"Detail\" class=\"btn btn-success btn-flat\"><i class=\"glyphicon glyphicon-eye-open\"></i></a> ".'<?=$edit;?> <?=$del;?>'."');
...
$list_table_off ='
                ...
                <section class="content">
                    <div class="row">
                        <div class="col-xs-12">
                            <div class="box">
                                <div class="box-header">
                                  <h3 class="box-title">Daftar '.ucwords($_POST['page_name']).'</h3>
                                </div><!-- /.box-header -->
                                <div class="box-body table-responsive">
       ...
       foreach ($db->fetch_all("sys_menu") as $isi) {
                      if ($path_url==$isi->url) {
                          if ($role_act["insert_act"]=="Y") {
                    ?>
          <a href="<?=base_index();?>'.str_replace("_", "-", $modul_name).'/tambah" class="btn btn-primary btn-flat"><i class="glyphicon glyphicon-plus-sign"></i> Tambah</a>
                          <?php
                          } 
                       } 
}
...
<div class="row">
    <div class="col-lg-12">
        <div class="box box-solid box-primary">
                                   <div class="box-header">
                                    <h3 class="box-title">Ubah '.ucwords($_POST['page_name']).'</h3>
                                    <div class="box-tools pull-right">
                     ...
                     <form id="update" method="post" class="form-horizontal" action="<?=base_admin();?>modul/'.$modul_name.'/'.$modul_name.'_action.php?act=up">
                      ...
                      <div class="form-group">
                        ...
                        <div class="col-lg-10">
						 ...
						 <a href="<?=base_index();?>'.str_replace("_", "-", $modul_name).'" class="btn btn-success btn-flat">Batal</a>
                        </div>
                      </div><!-- /.form-group -->
                    </form>
