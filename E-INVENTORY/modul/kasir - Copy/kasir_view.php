...
$cek=$db->fetch_custom("select max(NO_NOTA_PENJUALAN) as idMaks from penjualan");
...
window.location = "<?=base_index();?>kasir";
});
//batal penjualan
shortcut.add("F2",function() {
var result = confirm("Anda Yakin Membatalkan penjualan?");
if (result) {
 window.location = "<?=base_admin();?>modul/kasir/kasir_action.php?act=batal_penjualan&NO_NOTA_PENJUALAN=<?php echo $w.$IDbaru.'-'.$KODEOUTLETUSER;?>";
}
});
...
<script type="text/javascript">
	$(document).ready(function () {
			...
			var KODE_BARANG = $(this).val();
				$.ajax({
				url: "<?=base_admin();?>modul/kasir/cek_barang.php?KODE_BARANG="+KODE_BARANG
				}).done(function( data ) {
...
$.ajax({
type: "POST",
url: "<?=base_admin();?>modul/kasir/kasir_action.php?act=input_scanner",
data: dataString,
cache: true,
			...
			<?php if(isset($_GET["DAFTAR_BARANG"])){?>		
					 ...
					 <div class="col-lg-9" > 
						...
						<div class="btnbantuan" style="margin-top: -55px;">
							<a href="<?=base_index();?>kasir" style="border-radius:20px" class="btn btn-primary btn-flat"><i class="glyphicon glyphicon-minus-sign"></i>Tutup Pencarian (F5)</a>
							</div>
...
<td><?=$isi->NAMA_KATEGORI;?></td>
        ...
        <div class="box box-solid box-primary" style="height:635px;">
            ...
            <div class="col-xs-12">
             <h1 class="headingtable" style="margin-top:-4px" >
				...
				<b style="color:white"><?php echo date('Y-m-d H:i:s');?> &nbsp; <i class="glyphicon glyphicon-user"></i>&nbsp;<?=ucwords($db->fetch_single_row('sys_users','id',$_SESSION['id_user'])->username);?> / <?php echo $NAMA_OUTLET;?></small></b>
				  ...
				  <table id="kasir_dtb" class="table table-bordered table-striped" style="font-size:8pt">
                          <thead>
                          <tr>
                          <th style="width:25px" align="center">No</th>
                          <th>Kode barang</th>
			...
			$dtb=$db->fetch_custom("SELECT tmp_detail_penjualan.*,barang_outlet.* FROM tmp_detail_penjualan
		    INNER JOIN barang_outlet on barang_outlet.KODE_BARANG=tmp_detail_penjualan.KODE_BARANG");
			$i=1;
			foreach ($dtb as $isi) {
			?>
			<tr id="line_<?=$isi->KODE_BARANG;?>">
			...
			<a class="btn btn-danger"  href="<?=base_admin();?>modul/kasir/kasir_action.php?act=delete&id=<?=$isi->ID_TMP_PENJUALAN;?>"><i class="glyphicon glyphicon-trash"></i></a>
			</form>
              ...
              <div class="table-responsive">
                <table class="table" style="font-size:10pt;position:absolute;margin-top:-40px">
                 ...
                 <?php if($_POST["PELANGGAN_INPUT"]!=""){?>
				  <tr>
                    <th>Diskon pelanggan(<?php echo $VALUEDISKON_PELANGGAN;?>%)</th>
                    <td>Rp.<?php echo number_format($GETDISKONPELANGGAN);?></td>
                  </tr> 
				 <?php } ?>
                ...
                <small class="pull-right" style="margin-top:-20px">
				 ...
				 <div class="input-group input-group-sm" style="margin-top:-30px;margin-left:-200px">
                    ...
                    <select style="height:29px;width:166px"  name="PELANGGAN_INPUT" data-placeholder="Pilih Outlet penerima ..." class="form-control"  >
               <option value="">Pelanggan(Optional)</option>
			   
               <?php 
			   $nop=0;
			   foreach ($db->fetch_all("pelanggan") as $isi) {
				    $nop++;
               		echo "<option value='$isi->KODE_PELANGGAN'
					";
					if($isi->KODE_PELANGGAN==$KODE_PELANGGAN){echo "selected";}
					echo "
					>$nop. $isi->NAMA_PELANGGAN- Diskon($isi->DISKON%)</option>";
               } ?>
              </select><br/>
  ...
  <div class="col-lg-3" style="width:260px;height:260px" > 
					 ...
					 <div class="form-group">
                      ...
                      <div class="form-group" style="margin-top:-30px">
                        ...
                        <div class="col-lg-12" style="margin-left:35px">
						 ...
						 <a href="<?=base_index();?>kasir?DAFTAR_BARANG" class="btn btn-success btn-flat"><i class="glyphicon glyphicon-search"></i></a>
						   <a href="<?=base_index();?>kasir" class="btn btn-success btn-flat"><i class="glyphicon glyphicon-refresh"></i></a>
                        </div>
                      </div><!-- /.form-group -->
                    </form>
    ...
    <div class="col-lg-3" style="width:260px;margin-top:60px"> 
        <div class="box box-solid box-primary">
                  ...
                  <div class="box-body">
                     <form id="input" method="post" class="form-horizontal foto_banyak" action="<?=base_admin();?>modul/kasir/kasir_action.php?act=simpan_penjualan" >
                     <!-- /.form-group -->
						<div class="form-group">
                        <div class="col-lg-12">
						...
						<div class="form-group">
                         <div class="col-lg-12">
						  ...
						  <option value="DEBET">2.DEBET</option>
							</select>
                        </div>
                      </div>
					  ...
					  <div class="form-group" id="tampil_no" >
                        <div class="col-lg-12">
                          <input type="text" name="NO_KARTU" id="NO_KARTU"  placeholder="No Kartu" class="form-control" > 
                        </div>
                      </div>
					  ...
					  <div class="form-group">
                         <div class="col-lg-12">
                          ...
                          <input type="text" name="UANG_BAYAR" id="UANG_BAYAR" placeholder="Nominal Bayar..." onKeyPress="return goodchars(event,'0123456789',this)" onKeyup="hitung();" class="form-control" required> 
                        </div>
                      </div><!-- /.form-group -->
						<div class="form-group">
                        <div class="col-lg-12">
                          <input type="text" name="UANG_KEMBALI" id="UANG_KEMBALI" readonly placeholder="Nominal Kembali..." class="form-control" required> 
                        </div>
                      </div><!-- /.form-group -->
<!-- /.form-group -->
 <div class="form-group">
                      ...
                      <div class="form-group">
                        ...
                        <div class="col-lg-10">
                          <input type="submit" class="btn btn-success btn-flat" value="Selesai"> &nbsp;
						   <a href="<?=base_admin();?>modul/kasir/kasir_action.php?act=batal_penjualan&NO_NOTA_PENJUALAN=<?php echo $w.$IDbaru.'-'.$KODEOUTLETUSER;?>" class="btn btn-success btn-flat ">Batal</a>
