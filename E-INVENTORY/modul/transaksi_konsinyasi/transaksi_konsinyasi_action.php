...
switch ($_GET["act"]) {
	...
	if($_POST["TOTAL_TAGIHAN"]==0){
		echo "<script>alert('Belum ada barang yang dipilih');window.location='../../index.php/transaksi-konsinyasi'</script>";
		
	}
	if($_POST["TOTAL_TAGIHAN"]!=0){
    $data = array("NO_NOTA_KONSINYASI"=>$_POST["NO_NOTA_PENJUALAN"],"TANGGAL"=>$_POST["TANGGAL"],"OPERATOR_KASIR"=>$_POST["OPERATOR_KASIR"],"SUB_TOTAL_PENJUALAN"=>$_POST["TOTAL_TAGIHAN"],"TIPE_PEMBAYARAN"=>$_POST["TIPE_PEMBAYARAN"],"DISKON_PENJUALAN"=>$_POST["DISKON_PENJUALAN"],"TAX_SALES"=>$_POST["TAX_SALES"],"UANG_BAYAR"=>$_POST["UANG_BAYAR"],"UANG_KEMBALI"=>$_POST["UANG_KEMBALI"],"SUPPLIER"=>$_POST["PELANGGAN"],"CATATAN"=>$_POST["CATATAN"],"STATUS"=>$_POST["STATUS"],"DISKON_SUPPLIER"=>$_POST["DISKON_PELANGGAN"],"OUTLET"=>$KODEOUTLET,"JATUH_TEMPO"=>$_POST["JATUH_TEMPO"],"BIAYA_KIRIM"=>$_POST["BIAYA_KIRIM"],);
    $in_sales = $db->insert("transaksi_konsinyasi",$data);
    
			$gettmpjual=$db->fetch_custom("select * from tmp_detail_konsinyasi where NO_NOTA_KONSINYASI='$_POST[NO_NOTA_PENJUALAN]'");
			foreach ($gettmpjual as $isitmp){
			$tmpinput = array("NO_NOTA_KONSINYASI"=>$isitmp->NO_NOTA_KONSINYASI,"KODE_BARANG"=>$isitmp->KODE_BARANG,"HARGA_BARANG"=>$isitmp->HARGA_BARANG,"DISKON"=>$isitmp->DISKON,"JUMLAH"=>$isitmp->JUMLAH,"GRAND_PRICE"=>$isitmp->GRAND_PRICE,);
			...
			$inok = $db->insert("detail_konsinyasi",$tmpinput);	
			}
			$truncate=$db->fetch_custom("delete from tmp_detail_konsinyasi where NO_NOTA_KONSINYASI='$_POST[NO_NOTA_PENJUALAN]'");
		if ($in_sales=true) {
...
unlink($file);
echo "<script>alert('Penjualan Berhasil');window.location='../../index.php/transaksi-konsinyasi'</script>";
		}
		}
		else {
			return false;
		...
		case "batal_penjualan":
			...
			$truncate=$db->fetch_custom("delete from detail_konsinyasi where NO_NOTA_KONSINYASI=
			'$_GET[NO_NOTA_PENJUALAN]'");
		if ($in_sales=true) {
			header('location:../../index.php/transaksi-konsinyasi');
		} else {
			return false;
		}
		break;
	case "delete":
		$delete=$db->delete("tmp_detail_konsinyasi","ID_TMP_DETAIL_KONSINYASI",$_GET["id"]);
		if ($delete=true) {
			header('location:../../index.php/transaksi-konsinyasi');
		} else {
			return false; 
	...
	case "input_scanner":
		...
		$data = array("NO_NOTA_KONSINYASI"=>$_POST["NO_NOTA_PENJUALAN"],"KODE_BARANG"=>$barang->KODE_BARANG,"HARGA_BARANG"=>$barang->HARGA_JUAL,"DISKON"=>$barang->DISKON,"JUMLAH"=>$JUMLAH,"GRAND_PRICE"=>$jadi,);
		$in = $db->insert("tmp_detail_konsinyasi",$data);	
		...
		$itemchange = $db->fetch_single_row("tmp_detail_konsinyasi","ID_TMP_DETAIL_KONSINYASI",$value);
	    ...
	    $upa = $db->update("tmp_detail_konsinyasi",$data,"ID_TMP_DETAIL_KONSINYASI",$value);
		if ($upa=true) {
			header('location:../../index.php/transaksi-konsinyasi');
		} else {
			return false; 
