...
$cek=$db->fetch_custom("select max(NO_NOTA_KONSINYASI) as idMaks from transaksi_konsinyasi");
...
$DISKON_PELANGGAN = $db->fetch_single_row("supplier","ID_SUPPLIER",$_POST["PELANGGAN_INPUT"]);
$VALUEDISKON_PELANGGAN=$DISKON_PELANGGAN->DISKON;
$KODE_PELANGGAN=$DISKON_PELANGGAN->ID_SUPPLIER;
}
?>
...
window.location = "<?=base_index();?>transaksi-konsinyasi";
});
//batal penjualan
shortcut.add("F2",function() {
var result = confirm("Anda Yakin Membatalkan penjualan?");
if (result) {
 window.location = "<?=base_admin();?>modul/transaksi_konsinyasi/transaksi_konsinyasi_action.php?act=batal_penjualan&NO_NOTA_PENJUALAN=<?php echo $w.$IDbaru.'-'.$KODEOUTLETUSER;?>";
}
});
...
<script type="text/javascript">
	$(document).ready(function () {
			...
			var KODE_BARANG = $(this).val();
				$.ajax({
				url: "<?=base_admin();?>modul/transaksi-konsinyasi/cek_barang.php?KODE_BARANG="+KODE_BARANG
				}).done(function( data ) {
...
$.ajax({
type: "POST",
url: "<?=base_admin();?>modul/transaksi_konsinyasi/transaksi_konsinyasi_action.php?act=input_scanner",
data: dataString,
cache: true,
...
$("#flash").hide();
window.location='transaksi-konsinyasi';
$("#KODE_BARANG").focus();
}  
});
			...
			<?php if(isset($_GET["DAFTAR_BARANG"])){?>		
					 ...
					 <div class="col-lg-9" > 
						...
						<div class="btnbantuan" style="margin-top: -55px;">
							<a href="<?=base_index();?>transaksi-konsinyasi" style="border-radius:20px" class="btn btn-primary btn-flat"><i class="glyphicon glyphicon-minus-sign"></i>Tutup Pencarian (F5)</a>
							</div>
...
<td><?=$isi->NAMA_KATEGORI;?></td>
        ...
        <div class="box box-solid box-primary" style="height:640px;">
            ...
            <div class="col-xs-12">
             <h1 class="headingtable" style="margin-top:-4px" >
				...
				<b style="color:white"><?php echo date('Y-m-d H:i:s');?> &nbsp; <i class="glyphicon glyphicon-user"></i>&nbsp;<?=ucwords($db->fetch_single_row('sys_users','id',$_SESSION['id_user'])->username);?> / <?php echo $NAMA_OUTLET;?></small></b>
				  ...
				  <table id="transaksi-konsinyasi_dtb" class="table table-bordered table-striped" style="font-size:8pt">
                          <thead>
                          <tr>
                          <th style="width:25px" align="center">No</th>
                          <th>Kode barang</th>
			...
			$dtb=$db->fetch_custom("SELECT tmp_detail_konsinyasi.*,barang_outlet.* FROM tmp_detail_konsinyasi
		    INNER JOIN barang_outlet on barang_outlet.KODE_BARANG=tmp_detail_konsinyasi.KODE_BARANG");
			$i=1;
			foreach ($dtb as $isi) {
			?>
			<tr id="line_<?=$isi->KODE_BARANG;?>">
			...
			<td><?=$isi->DISKON;?> %</td>
			 <form  method="post" class="form-horizontal foto_banyak" action="<?=base_admin();?>modul/transaksi_konsinyasi/transaksi_konsinyasi_action.php?act=ubah_scanner">
			 <input type="hidden" id="ID_TMP_PENJUALAN" name="ID_TMP_PENJUALAN[]" value="<?=$isi->ID_TMP_DETAIL_KONSINYASI;?>" >
			...
			<a class="btn btn-danger"  href="<?=base_admin();?>modul/transaksi_konsinyasi/transaksi_konsinyasi_action.php?act=delete&id=<?=$isi->ID_TMP_DETAIL_KONSINYASI;?>"><i class="glyphicon glyphicon-trash"></i></a>
			</form>
            ...
            <div class="col-xs-12">
              ...
              <div class="table-responsive">
                ...
                <small class="pull-right" >
                    ...
                    <input type="text" name="global_discount" onChange="this.form.submit()"   id="global_discount"  value="<?php 
               ...
               <?php 
			   $nop=0;
			   foreach ($db->fetch_all("supplier") as $isi) {
				    $nop++;
               		echo "<option value='$isi->ID_SUPPLIER'
					";
					if($isi->ID_SUPPLIER==$KODE_PELANGGAN){echo "selected";}
					echo "
					>$nop. $isi->NAMA_SUPPLIER</option>";
               } ?>
              </select><br/>
  ...
  <div class="col-lg-3" style="width:260px;height:260px" > 
					 ...
					 <div class="form-group">
                      ...
                      <div class="form-group" style="margin-top:-30px">
                        ...
                        <div class="col-lg-12" style="margin-left:35px">
						 ...
						 <a href="<?=base_index();?>transaksi-konsinyasi?DAFTAR_BARANG" class="btn btn-success btn-flat"><i class="glyphicon glyphicon-search"></i></a>
						   <a href="<?=base_index();?>transaksi-konsinyasi" class="btn btn-success btn-flat"><i class="glyphicon glyphicon-refresh"></i></a>
                        </div>
                      </div><!-- /.form-group -->
                    </form>
    ...
    <div class="col-lg-3" style="width:260px;margin-top:60px"> 
        <div class="box box-solid box-primary">
                  ...
                  <div class="box-body">
                     <form id="input" method="post" class="form-horizontal foto_banyak" action="<?=base_admin();?>modul/transaksi_konsinyasi/transaksi_konsinyasi_action.php?act=simpan_penjualan" >
                     <!-- /.form-group -->
						<div class="form-group">
                        <div class="col-lg-12">
                          ...
                          <input type="hidden" name="OPERATOR_transaksi-konsinyasi" value="<?php echo $_SESSION['id_user'];?>" class="form-control">
                      ...
                      <div class="form-group">
                        ...
                        <div class="col-lg-10">
                          <input type="submit" class="btn btn-success btn-flat" value="Selesai"> &nbsp;
						   <a href="<?=base_admin();?>modul/transaksi-konsinyasi/transaksi-konsinyasi_action.php?act=batal_penjualan&NO_NOTA_PENJUALAN=<?php echo $w.$IDbaru.'-'.$KODEOUTLETUSER;?>" class="btn btn-success btn-flat ">Batal</a>
